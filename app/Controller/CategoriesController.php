...
class CategoriesController extends AppController {
	...
	function add() {
		...
		if (!empty($this->request->data)) {
			...
			if(!empty($this->request->data['Category']['category_image']['name'])):
			
			$image_path = $this->Image->upload_image_and_thumbnail($this->request->data,"category_image",800,800,"categories",true,'Category');
			$this->request->data['Category']['category_image']=$image_path;
			
			else:
				unset($this->request->data['Category']['category_image']);
			endif;
	...
	function edit($id = null) {
		...
		if (!empty($this->request->data))
		{
			...
			$image_path = $this->Image->upload_image_and_thumbnail($this->request->data,"category_image",573,150,"categories",true,'Category');
			$this->request->data['Category']['category_image']=$image_path;
		...
		$parentCategories = $this->Category->find('list',array('fields'=>'id,category_name','conditions'=>array('store_id'=>$this->request->data['Category']['store_id'])));
	...
	{
		if(!empty($this->request->data)):
			$store_id=$this->request->data['Category']['store_id'];
