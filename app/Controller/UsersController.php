...
class UsersController extends AppController {
	...
	function login() {
		if ($this->Session->read('Auth.User')) {
			...
			return $this->redirect('users', null, false);
	...
	function add() {
		if (!empty($this->request->data)) {
			...
			$this->request->data['User']['password']=!empty($this->request->data['User']['new_password']) ? $this->Auth->password($this->request->data['User']['new_password']) : null;
			
			$this->request->data['User']['confirmPassword']=!empty($this->request->data['User']['confirm_password']) ? $this->Auth->password($this->request->data['User']['confirm_password']) : null;
			...
			if ($this->User->save($this->request->data)) {
				...
				if($this->request->data['User']['group_id'] == '2') {
					...
					$storedata['Store']['store_name'] =  $this->request->data['User']['username']." "."Store";
		  ...
		  {				
			...
			if ($this->User->save($this->request->data)) {
				//Create  Store
					...
					$this->loadModel('Store');
					$storedata['Store']['user_id'] =  $user_id;
					$storedata['Store']['store_name'] =  $this->request->data['User']['StoreName'];
					$storedata['Store']['store_url'] = $this->request->data['User']['storeUrl'];
					$this->Store->save($storedata);
				   ...
				   $this->request->data['MerchantPaymentDetail']['plan_id']=$this->request->data['User']['SubscriptionPlan'];
				   $this->request->data['MerchantPaymentDetail']['user_id']=$user_id;
				   $this->request->data['MerchantPaymentDetail']['payment_status']='inactive';
				   if($this->MerchantPaymentDetail->save($this->request->data))
					{				  
					   $paymentId = $this->MerchantPaymentDetail->getInsertID(); 
                         #set the variables to send to the paypal
                        $planId =$this->request->data['User']['SubscriptionPlan'];
